...
int packagesPage::fixCriticals(QString package)
{
    for ( int i =0; i < packages.count(); i++)
    {
        for ( int k =0; k < packages.at(i).second.count(); k++)
        {
            if ( (packages.at(i)).second.at(k).Name == package)
            {
                ...
                {
                    fixCriticals((packages[i]).second[k].Depends.at(l));
                }
            }
        }
    }
...
int packagesPage::initPackages()
{
    ...
    {
        //fixCriticals(criticSubDeps.at(l));
    }
    return 0;
}
...
meskPackage packagesPage::parsePackageFile(const QString& packageFilePath)
{
    ...
    for ( int i =0; i < 5; i++)
    {
        ...
        {
            while(buffer.size() != 0 && !packageFile.atEnd())
            {
...
int packagesPage::updatePackageList(QListWidgetItem* item)
{
    ...
    {
        for ( int j =0; j < packagesList.at(item->listWidget()->currentRow()).second.size(); j++)
        //for ( int j =0; j < (packagesList.at(item)).second.size(); j++)
        {
            if (packagesList.at(item->listWidget()->currentRow()).second.at(j)->flags()&Qt::ItemIsUserCheckable)
            {
                packagesList.at(item->listWidget()->currentRow()).second.at(j)->setCheckState(Qt::Checked);
            }
    ...
    else if (item->checkState() == Qt::Unchecked)
    {
        ...
        //for ( int j =0; j < (packagesList.at(item)).second.size(); j++)
        {
            ...
            if (packagesList.at(item->listWidget()->currentRow()).second.at(j)->flags()&Qt::ItemIsUserCheckable)
            {
                packagesList.at(item->listWidget()->currentRow()).second.at(j)->setCheckState(Qt::Unchecked);
            }
...
QVector<meskPackage> packagesPage::getSelectedPackages()
{
    ...
    for  (int i =0; i < packagesList.size(); i++)
    {
        ...
        for (int j =0; j < packagesList.at(i).second.size(); j++)
        {
            ...
            {
                totalSize += packages.at(i).second.at(j).Size;
                selectedPackages.push_back(packages.at(i).second.at(j));
            }
        }
    }
